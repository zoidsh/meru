...
describe("extractVerificationCode", () => {
  ...
  test("returns null when context exists but no code is found", () => {
    expect(extractVerificationCode(["verification code", "no numbers here"])).toBeNull();
  });

  test("extracts 4-digit numeric code with verification context", () => {
    expect(extractVerificationCode(["Your verification code is", "1234"])).toBe("1234");
  });
  ...
  test("extracts 8-digit numeric code with verification context", () => {
    expect(extractVerificationCode(["sign-in code:", "12345678"])).toBe("12345678");
  });
  ...
  test("matches 'sign-in' context with hyphen", () => {
    expect(extractVerificationCode(["sign-in verification", "9012"])).toBe("9012");
  });
  ...
  test("prefers numeric code over alphanumeric code", () => {
    expect(extractVerificationCode(["verification", "AB12CD", "567890"])).toBe("567890");
  });
  ...
  test("extracts code from middle of text", () => {
    expect(extractVerificationCode(["verification", "Your code is 7890 here"])).toBe("7890");
  });

  test("extracts first valid code when multiple exist", () => {
    expect(extractVerificationCode(["verification", "1234", "5678"])).toBe("1234");
  });

  test("requires alphanumeric code to have at least one letter", () => {
    expect(extractVerificationCode(["verification", "12345678"])).toBe("12345678");
  });
  ...
  test("handles code in same text as context", () => {
    expect(extractVerificationCode(["Your verification code is 9999"])).toBe("9999");
  });
