...
/**
 * The config ladder, kept out of `config.ts` so that it can be exercised
...
} satisfies NonNullable<Options<Config>["migrations"]>;

/**
 * The conf options the config store is built from, minus the ones only the main
 * process can fill in. Shared with the tests so that they run the ladder under
 * the same key handling and the same resolved version the app does.
 */
export function createConfigOptions({
  ...
  Options<Config>,
  "accessPropertiesByDotNotation" | "projectVersion" | "defaults" | "migrations"
> {
  return {
    accessPropertiesByDotNotation: false,
    // electron-store fills this with `app.getVersion()`, whose prerelease suffix
    // on a Beta build misses every key in the ladder.
    projectVersion: resolveMigrationVersion(version),
    defaults,
