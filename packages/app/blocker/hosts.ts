...
// Email open-tracking pixels. Split by how they match: literal host/path fragments go
// in one combined regexp of alternatives that V8 can prefilter (the fast path), while
// the few entries that need real regexp syntax are kept separate so their unbounded
// quantifiers don't defeat that prefilter for the whole set.
const EMAIL_TRACKER_SUBSTRINGS = [
  // 365offers
  "trk.365offers.trade",
  // Absolutesoftware
...
export function createBlockMatcher({ ads, tracking }: { ads: boolean; tracking: boolean }) {
  ...
  if (hosts.length) {
    patterns.push(createHostBoundaryPattern(hosts));
  }
