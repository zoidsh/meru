...
describe("buildScopedSheetText", () => {
  test("emits variable definitions before rules inside one @scope block", () => {
    const sheetText = buildScopedSheetText({
      scopeSelector: '[data-dark-theme-root^="0-"]',
      scopeLimitSelector: null,
      variableDefinitions: [
    ...
    expect(sheetText).toBe(
      [
        '@scope ([data-dark-theme-root^="0-"]) {',
        ":scope { --surface: #181a1b; }",
        ".surface { background-color: #181a1b; }",
        "}",
      ].join("\n"),
    );
  });

  test("adds the scope limit and mirrors importance", () => {
    const sheetText = buildScopedSheetText({
      scopeSelector: '[data-dark-theme-root^="0-"]',
      scopeLimitSelector: ".chip, .badge",
      variableDefinitions: [],
      rules: [
        createSheetRule({
          declarations: [
    ...
    expect(sheetText).toBe(
      [
        '@scope ([data-dark-theme-root^="0-"]) to (.chip, .badge) {',
        ".surface { color: #e8e6e3 !important; }",
        "}",
      ].join("\n"),
    );
  });
  ...
  test("dedupes identical generated rules and skips declaration-less rules", () => {
    ...
    expect(sheetText).toBe(["@scope ([data-x]) {", ".surface { color: #e8e6e3; }", "}"].join("\n"));
  });
...
describe("buildInstanceSheetText", () => {
  test("combines root background, invert rules, and counter rules in one scope", () => {
    const instanceSheetText = buildInstanceSheetText({
      scopeSelector: '[data-dark-theme-root="0-1"]',
      scopeLimitSelector: ".edeTZ",
    ...
    expect(instanceSheetText).toBe(
      [
        '@scope ([data-dark-theme-root="0-1"]) to (.edeTZ) {',
        ":scope { background-color: rgb(19, 19, 19); }",
        ".icon { filter: invert(1); }",
        ":is(.dC):is(.dC) { background-color: rgb(194, 231, 255); }",
        "}",
      ].join("\n"),
    );
  });
