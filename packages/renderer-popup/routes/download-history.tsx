...
function DateFromNow({ timestamp }: { timestamp: number }) {
  ...
  return <div className="text-muted-foreground first-letter:capitalize">{timeAgo}</div>;
}

export function DownloadHistoryClearAllButton() {
...
export function DownloadHistoryContent() {
  ...
  return (
    <div className="space-y-4">
      {config["downloads.history"].map(({ id, fileName, filePath, createdAt }) => (
        <Card key={id}>
          <CardContent className="flex gap-4 items-center text-sm">
            ...
            <div className="flex gap-2">
              ...
              <Tooltip delayDuration={700}>
                ...
                <TooltipContent side="bottom">Remove from history</TooltipContent>
              </Tooltip>
            </div>
          </CardContent>
        </Card>
