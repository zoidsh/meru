...
- `TODO.md` (at the repo root) is a lean list of open items to pick up in a **new session** — work unrelated enough to the current feature's goal that it shouldn't ride along with it. Keep entries short and link to `docs/` for context. It is not a backlog for the in-progress feature, and it holds no knowledge: settled decisions go to `docs/decisions.md`, feature roadmaps and handoffs to `docs/features/`.
