...
function buildAppFiles() {
    ...
    const buildPreloadFile = (preloadName) => rolldown({
        ...rolldownOptions,
...
async function buildRenderer(rendererName, port) {
    const viteConfig = {
        configFile: false,
...
if (args.values.dev) {
    ...
    for await (const event of watcher) {
        if (["renderer/", "renderer-popup/", "renderer-lib/", "ui/"].some((pathname) => event.filename?.includes(pathname))) {
            continue;
        }
        await buildAppFiles();
