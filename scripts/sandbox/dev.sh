#!/usr/bin/env bash
# Entry point for `bun run dev:sandbox`. Runs the dev server on the host and
# the app in a container, for machines that cannot run Electron directly.
set -euo pipefail

if [ "$(uname -s)" != "Linux" ]; then
  echo "dev:sandbox runs the app in a Linux container and only works on Linux." >&2
...
if ! command -v docker > /dev/null; then
  echo "dev:sandbox needs Docker to run the app, and docker is not on PATH." >&2
...
export ELECTRON_OVERRIDE_DIST_PATH="$PWD/scripts/sandbox"
